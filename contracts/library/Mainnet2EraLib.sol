...
library Mainnet2EraLib {
    function parsePublicInputSource(
    ...
    ) internal pure returns (HelperLib.PublicInputDataSource memory) {
        ...
        return
            HelperLib.PublicInputDataSource({
                tx_hash: bytes32(
                    (uint256(
                        ...
                        uint256(
                            bytes32(
                                proofData[TransactionSplitStart + SplitStep * 3:TransactionSplitStart + SplitStep * 4]
                            )
                        )
                ),
                chain_id: uint64(
