library StringLib {
    ...
    function getSrouceValue3Args(uint value, uint decimalPlaces) internal pure returns (uint256, uint256, uint256) {
        uint256 decimalPart = value % (10 ** decimalPlaces);
        // Convert the integer part to a string
        // If there is no decimal part, return just the integer part
        if (decimalPart == 0) {
        ...
        // Convert the decimal part to a string
        string memory decimalPartString = StringLib.removeTrailingZeros(StringLib.toString(decimalPart));
        bytes memory bstr = bytes(decimalPartString);
        uint256 index = 0;
        for (uint i = 0; i < bstr.length; i++) {
            if (bstr[i] == 0x39 && bstr.length - i <= 9) {
                index = i;
                break;
            }
        }
        decimalPartString = StringLib.substring(decimalPartString, index, bstr.length);
        uint maxLength = bytes(decimalPartString).length;
        uint chainId = StringLib.parseInt(StringLib.substring(decimalPartString, 0, 4));
        uint ebcId = StringLib.parseInt(StringLib.substring(decimalPartString, 4, 6));
        uint dealerId = StringLib.parseInt(StringLib.substring(decimalPartString, 6, maxLength));
        if (maxLength == 7) {
            dealerId *= 10;
        }
        return (chainId, ebcId, dealerId);
    }
}
