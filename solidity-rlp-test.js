...
contract('RLPReader', async (accounts) => {
  ...
  it('detects the entire byte length of an RLP item', async () => {
    ...
    assert(
      result.toNumber() == str.length,
      `[1, 2, 3] should only take ${str.length} bytes to rlp encode`,
    )
    ...
    assert(
      result.toNumber() == str.length,
      `list of 1024 a characters should only take ${str.length} bytes to rlp encode`,
    )
    ...
    assert(
      result.toNumber() == str.length,
      `the number 1 should only take ${str.length} bytes to rlp encode`,
    )
    ...
    assert(
      result.toNumber() == str.length,
      `string of 1024 a characters should only take ${str.length} bytes to rlp encode`,
    )
    ...
    assert(
      result.toNumber() == str.length,
      `string of ${len} a characters should only take ${str.length} bytes to rlp encode`,
    )
    ...
    assert(
      result.toNumber() == str.length,
      'Incorrect calculated rlp item byte length for nested structure',
    )
    ...
    assert(
      result.toNumber() == 1,
      'Incorrect calculate rlp item byte length for empty list',
    )
  })
  ...
  it('returns the correct payload memory offset and length', async () => {
    ...
    assert(
      result.payloadLen.toNumber() == 1,
      'incorrect payload length of a single byte encoding',
    )
    assert(
    ...
    result = await helper.payloadLocation.call(
      toHex(rlp.encode(toHex(Array(36).fill(0).join('')))),
    )
    assert(
      result.payloadLen.toNumber() == 18,
      'incorrect payload length of a 18 bytes',
    )
    assert(
    ...
    result = await helper.payloadLocation.call(
      toHex(rlp.encode(toHex(Array(200).fill(0).join('')))),
    )
    assert(
      result.payloadLen.toNumber() == 100,
      'incorrect payload length of a 100 bytes',
    )
    assert(
  ...
  it('detects the correct amount of items in a list', async () => {
    let assertString =
      'Number of items in an rlp encoded list wrongly detected: '
    let str = [1, 2, 3]
    ...
    assert(
      result.toNumber() == str.length,
      assertString + 'string of 1024 `a` characters',
    )
  ...
  it('properly handles data conversions', async () => {
    ...
    assert(
      result == str,
      'Incorrect toBytes conversion for bytes longer than 1 evm word',
    )
    ...
    result = await helper.toUint.call(
      toHex(rlp.encode(toHex(Array(64).fill(0).join('')))),
    )
    ...
    assert(
      result[1].toNumber() == str[0][1],
      'Nested second element incorrectly decoded',
    )
  ...
  it("correctly converts an rlpItem to it's raw byte from", async () => {
    ...
    assert(
      toHex(str) == toHex(result),
      'incorrectly converted to the raw byte form',
    )
  ...
  it('catches bad input', async () => {
    ...
    if (!err) assert.fail(null, null, 'converted an address less than 20 bytes')
    ;[err] = await catchError(
      helper.toAddress.call(toHex(rlp.encode(accounts[0] + '1'))),
    )
    if (!err)
    ...
    ;[err] = await catchError(
      helper.toUint.call(toHex(rlp.encode(toHex(Array(66).fill(0).join(''))))),
    )
    if (!err) {
    ...
    if (!err) {
      assert.fail(
        null,
        null,
        'converted a uint without padding to 32 bytes with strict enforcement',
      )
    }
  ...
  it('correctly creates an iterator', async () => {
    ...
    if (!err)
      assert.fail(
        null,
        null,
        'constructed an iterator from something not a list',
      )
    ...
    if (err)
      assert.fail(
        null,
        null,
        'could not construct iterator out of a valid list',
      )
  ...
  it('correctly iterates over an RLP list (e.g., an RLP encoded block header)', async () => {
    ...
    const block = {
      parentHash:
        '0x487e074bba7f0749950d7e2f226307c8ac388cb0410cfe817931a5a44077e159',
      sha3Uncles:
        '0x1dcc4de8dec75d7aab85b567b6ccd41ad312451b948a7413f0a142fd40d49347',
      stateRoot:
        '0x7b814195793c699d345339dd7a4225112ad91b9ba7f03787563a9e98ba692e52',
      transactionsRoot:
        '0xad6c9f611dfdd446855cb430b8392e20538db4f0349336063e710c6c483e9e43',
      receiptsRoot:
        '0xf022ddad6e90316614df496922cb73508a9abecfda2d3076a5f1129a01497869',
      ...
      mixHash:
        '0x8a24dc2c8fb497ff40a622173d9c7804a274de3da4b335b2ba0e3c53e3fae714',
    ...
    assert(
      result.transactionsRoot == block.transactionsRoot,
      'transactionsRoot not equal',
    )
    ...
    assert(
      result.nonce.toString() == web3.utils.toBN(block.nonce).toString(),
      'nonce not equal',
    )
  })
  ...
  it('payloadKeccak256 for empty payload is the same as keccak256(new bytes(0))', async () => {
    ...
    const emptyListDataHash = await helper.payloadKeccak256.call(
      emptyListRlpBytes,
    )
    assert.equal(emptyListDataHash, EMPTY_BYTES_HASH, 'empty list')

    const emptyBytesRlpBytes = rlp.encode(Buffer.alloc(0)) // 0x80
    const emptyBytesDataHash = await helper.payloadKeccak256.call(
      emptyBytesRlpBytes,
    )
